...
const hideUnchangedRegions = ref<boolean>(false)
watch(()=>hideUnchangedRegions.value, (value: boolean) => {
...
const initDiffEditor = () => {
  editor = monaco.editor.createDiffEditor(editorRef.value, {
    ...
    wordWrap: 'off', // 自动换行
    fontSize: 14, // 字体大小
    renderIndicators: false, // 为添加/删除的更改呈现+/-指示器.Defaults to true.
    hideUnchangedRegions: {
...
<template>
  <a-switch v-model:checked="hideUnchangedRegions" />
  <div ref="editorRef" v-bind="$attrs" style="height: calc(100vh - 154px)"/>
</template>
